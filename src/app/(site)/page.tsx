...
export default function Home() {
  ...
  const calculateProgress = () => {
    ...
    const percentage = (queue / getLeads.length) * 100;
    return percentage.toFixed(1) + "%";
  };

  return (
    <div className="w-ful h-screen overscroll-y-auto px-12 pb-6 pt-4">
      <div className="flex items-baseline justify-between py-4">
        ...
        <div className="mb-5 flex -space-x-4">
          {getLeads.slice(0, 7)?.map((lead: any, idx: number) => (
            <Avatar
              size="w-10 h-10"
              url={lead.avatar}
              key={idx}
              length={getLeads.length}
              last={idx === 6}
            />
          ))}
        </div>
